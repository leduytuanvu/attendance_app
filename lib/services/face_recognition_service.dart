...
class FaceRecognitionService {
  ...
  final _faceDetector = FaceDetector(
    options: FaceDetectorOptions(
      ...
      enableClassification:
          true, // Enable face classification (smiling, eyes open)
      performanceMode:
      ...
      try {
        ...
        try {
          ...
          if (modelData.lengthInBytes < 1000) {
            print(
                'Model file is too small, might be corrupted: ${modelData.lengthInBytes} bytes');
          ...
          try {
            ...
            _isInitialized = true;
            print(
                'Face recognition model initialized successfully directly from assets');
            return;
          } catch (bufferError) {
          ...
          if (!await tempFile.exists() || await tempFile.length() < 1000) {
            print(
                'Temp file is invalid or too small: ${await tempFile.length()} bytes');
          ...
          print(
              'Face recognition model initialized successfully from temp file');
        } catch (e) {
  ...
  Future<File> _loadModelToTempFile() async {
    ...
    try {
      ...
      print(
          'Model loaded successfully from assets and saved to temporary file');
      return tempFile;
    } catch (e) {
  ...
  Future<List<double>?> getFaceEmbedding(
    ...
    if (!_isInitialized) {
      ...
      if (!_isInitialized) {
        print(
            'Face recognition model not initialized, using fallback behavior');
        return null;
      }
    }
    ...
    try {
      ...
      final outputTensor =
          List<double>.filled(embeddingSize, 0).reshape([1, embeddingSize]);
  ...
  Future<List<List<List<List<double>>>>?> _preprocessFace(
      CameraImage cameraImage, Face face) async {
    try {
      ...
      final faceRect =
          _getPaddedFaceRect(face.boundingBox, image.width, image.height);
  ...
  Rect _getPaddedFaceRect(Rect faceRect, int imageWidth, int imageHeight) {
    ...
    final bottom =
        (centerY + paddedHeight / 2).clamp(0.0, imageHeight.toDouble());
  ...
  bool isFaceMatch(List<double> embedding1, List<double> embedding2,
  ...
  Map<String, dynamic> findBestMatch(List<double> queryEmbedding,
      Map<String, List<double>> registeredEmbeddings,
      {double threshold =
          0.94} // Increased threshold to require 92% similarity for a match
      ) {
    String bestMatchId = '';
    ...
    if (allMatches.isNotEmpty) {
      ...
      for (int i = 0; i < math.min(3, allMatches.length); i++) {
        print(
            '[leduytuanvu] Match #${i + 1}: ID=${allMatches[i]['id']}, Score=${allMatches[i]['score']}');
      }
    }
  ...
  Map<String, dynamic> findBestGeometricMatch(
      String queryFaceData, Map<String, String> registeredFaceData,
      {double threshold =
          0.94} // Increased threshold to require 92% similarity for a match
      ) {
    String bestMatchId = '';
    ...
    for (final entry in registeredFaceData.entries) {
      ...
      final similarity =
          _compareGeometricFaceData(queryFaceParams, registeredFaceParams);
  ...
  double _compareGeometricFaceData(
      Map<String, double> face1, Map<String, double> face2) {
    ...
    final positionSimilarity = _calculatePositionSimilarity(
        face1['left']!, face1['top']!, face2['left']!, face2['top']!);

    // 3. Angle similarity (face orientation)
    final angleSimilarity = _calculateAngleSimilarity(
        face1['angleY']!, face1['angleZ']!, face2['angleY']!, face2['angleZ']!);
    ...
    final combinedSimilarity = sizeSimilarity * 0.4 +
  ...
  double _calculateSizeSimilarity(
    ...
    ratio = math
        .pow(ratio, 0.5)
        .toDouble(); // Square root makes the curve more lenient
    ...
    final aspectRatioSimilarity =
        1.0 - math.min(aspectRatioDiff / 0.3, 1.0); // More lenient threshold

    // Print detailed size similarity info for debugging
    print(
        '[leduytuanvu] Size details: Area1=$area1, Area2=$area2, Ratio=$ratio');
    print(
        '[leduytuanvu] Aspect ratio details: AR1=$aspectRatio1, AR2=$aspectRatio2, Similarity=$aspectRatioSimilarity');
  ...
  double _calculatePositionSimilarity(
      double left1, double top1, double left2, double top2) {
    ...
    print(
        '[leduytuanvu] Position details: Left1=$left1, Left2=$left2, HDist=$horizontalDistance');
    print(
        '[leduytuanvu] Position details: Top1=$top1, Top2=$top2, VDist=$verticalDistance');
    ...
    final horizontalSimilarity =
        1.0 - (horizontalDistance / maxHorizontalDistance).clamp(0.0, 1.0);
    final verticalSimilarity =
        1.0 - (verticalDistance / maxVerticalDistance).clamp(0.0, 1.0);

    // Apply a non-linear transformation to make the similarity score more forgiving
    // for small differences but still penalize large differences
    final horizontalSimilarityAdjusted =
        math.pow(horizontalSimilarity, 0.7).toDouble();
    final verticalSimilarityAdjusted =
        math.pow(verticalSimilarity, 0.7).toDouble();
    ...
    final combinedSimilarity =
        horizontalSimilarityAdjusted * 0.4 + verticalSimilarityAdjusted * 0.6;

    print(
        '[leduytuanvu] Position similarity: H=$horizontalSimilarityAdjusted, V=$verticalSimilarityAdjusted, Combined=$combinedSimilarity');
  ...
  double _calculateAngleSimilarity(
      double angleY1, double angleZ1, double angleY2, double angleZ2) {
    ...
    print(
        '[leduytuanvu] Angle details: Y1=$angleY1, Y2=$angleY2, YDiff=$yDiff');
    print(
        '[leduytuanvu] Angle details: Z1=$angleZ1, Z2=$angleZ2, ZDiff=$zDiff');
    ...
    final combinedSimilarity =
        ySimilarityAdjusted * 0.7 + zSimilarityAdjusted * 0.3;

    print(
        '[leduytuanvu] Angle similarity: Y=$ySimilarityAdjusted, Z=$zSimilarityAdjusted, Combined=$combinedSimilarity');
  ...
  Future<List<double>?> processImageFile(File imageFile) async {
    ...
    if (!_isInitialized) {
      ...
      if (!_isInitialized) {
        print(
            'Face recognition model not initialized, using fallback behavior for image file');
        return null;
      }
    }
    ...
    try {
      ...
      final faceRect = _getPaddedFaceRect(
          largestFace.boundingBox, image.width, image.height);
